...
{
    private readonly VdfSerializerSettings settings;
    ...
    public VdfSerializer(VdfSerializerSettings settings)
    {
        ...
        if (this.settings.UsesConditionals && this.settings.DefinedConditionals == null)
    ...
    private VProperty ReadProperty(VdfReader reader)
    {
        ...
        else
        {
            throw new VdfException(
                $"Unexpected state when deserializing property (key: {result.Key}, state: {reader.CurrentState}).");
        }
    ...
    private VObject ReadObject(VdfReader reader)
    {
        ...
        while (!(reader.CurrentState == VdfReader.State.Object && reader.Value == VdfStructure.ObjectEnd.ToString()))
            ...
            else if (reader.CurrentState == VdfReader.State.Property)
            {
                ...
                if (!settings.UsesConditionals || prop.Conditional == null ||
                    prop.Conditional.Evaluate(settings.DefinedConditionals!))
                    result.Add(prop);
            }
            else
            {
                throw new VdfException(
                    $"Unexpected state when deserializing (state: {reader.CurrentState}, value: {reader.Value}).");
            }
    ...
    private VConditional ReadConditional(VdfReader reader)
    {
        ...
        while (reader.CurrentState == VdfReader.State.Conditional &&
               reader.Value != VdfStructure.ConditionalEnd.ToString())
        {
            if (reader.Value == "!")
