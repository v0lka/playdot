...
public class VObject : VToken, IList<VToken>, IDictionary<string, VToken>
{
    ...
    public override VToken? this[object key]
    {
        get
        {
            ...
            if (!(key is string propertyName))
                throw new ArgumentException(
                    $"Accessed JObject values with invalid key value: {MiscellaneousUtils.ToString(key)}. Object property name expected.");
        ...
        set
        {
            ...
            if (!(key is string propertyName))
                throw new ArgumentException(
                    $"Set JObject values with invalid key value: {MiscellaneousUtils.ToString(key)}. Object property name expected.");
    ...
    protected override bool DeepEquals(VToken token)
    {
        ...
        return _children.Count == otherObj._children.Count && Enumerable.Range(0, _children.Count)
            .All(x => DeepEquals(_children[x], otherObj._children[x]));
    }
